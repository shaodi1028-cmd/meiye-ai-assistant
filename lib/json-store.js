...
const { buildExportPayload, dataError, validateImportPayload } = require("./store-utils");
...
function createJsonStore(options = {}) {
  ...
  function resetFromSeed(reason = "before-reset") {
    ...
    let backupPath = "";
    if (fs.existsSync(dbPath)) {
      backupPath = createBackup(read(), reason);
    }
